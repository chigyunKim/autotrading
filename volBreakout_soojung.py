...
def get_current_cash():
    ...
    cpCash.BlockRequest()
    return cpCash.GetHeaderValue(45)  # 증거금 100% 주문 가능 금액
...
def get_target_price(code):
    ...
    try:
        ...
        target_price = today_open + (lastday_high - lastday_open) * 0.45
        return target_price
        ...
        while True:
            ...
            for s in stocks:
                if s['qty'] != 0:
                    ...
                    cpOrder.SetInputValue(8, "03")  # 호가 12:최유리, 13:최우선, 03:시장가
                    ret = cpOrder.BlockRequest()
                    printlog('시장가 매도', s['code'], s['name'], s['qty'],
...
if __name__ == '__main__':
    try:
        ...
        buy_percent = 1 / len(symbol_list) -0.02
        ...
        while True:
            t_now = datetime.now()
            t_0850 = t_now.replace(hour=8, minute=55, second=0, microsecond=0)
            t_9 = t_now.replace(hour=9, minute=0, second=0, microsecond=0)
            t_start = t_now.replace(hour=9, minute=0, second=15, microsecond=0)
            t_last = t_now.replace(hour=15, minute=29, second=59, microsecond=0)
            t_exit = t_now.replace(hour=15, minute=31, second=0, microsecond=0)
            today = datetime.today().weekday()
            if today == 5 or today == 6:  # 토요일이나 일요일이면 자동 종료
                printlog('Today is', 'Saturday.' if today == 5 else 'Sunday.')
                sys.exit(0)
            if t_0850 < t_now < t_9 and soldout == False: # AM 08:55 ~ AM 09:00 : 전날 매수된 게 있으면 매도
                soldout = True
                sell_all()
            if t_start < t_now < t_last:  # AM 09:00:15 ~ PM 03:30 : 매수
                ...
                if t_now.minute == 30 and 0 <= t_now.second <= 5:
                    ...
                    time.sleep(5)
            '''
            if t_last < t_now < t_exit:  # PM 03:15 ~ PM 03:20 : 일괄 매도
                if sell_all() == True:
